...
```{r}
# Anova Table
res.aov2 <- aov(as.numeric(y) ~ job + education + campaign + pdays + poutcome + nr.employed + emp.var.rate, data = imputed)
summary(res.aov2)
```
...
m2 <- lm(as.numeric(y) ~ job + education + campaign + pdays + poutcome + nr.employed + emp.var.rate, data = logit.os.Train)
l2 <- multcomp::glht(m2, linfct = mcp(y = "Tukey"))
summary(l2)
```
