...
### Logit - Full Model - Original data 90.8% (DUMP?)
```{r}
set.seed(1234)
...
### Logit - Full Model - SMOTE 50- 50 Sampled data 86.6% (DUMP?)
```{r}
set.seed(1234)
...
### Logit - Full Model - ROSE 85.5% (DUMP?)
```{r}
set.seed(1234)
...
obj1.mod.acc <- 0.8072
obj1.mod.sens <- 0.8373
obj1.mod.spec <- 0.5704

# Goodness of fit test
hl <- hoslem.test(red.mod7.log$y, fitted(red.mod7.log ), g=10)
hl
conf.step<-table(class.step,logit.os.Test$y)
chisq.test(conf.step)
...
residuals2 <- step.os.log$residuals
write.table(residuals2, file = "residuals.txt", sep = "\t")
...
red.mod7.log <- glm(y ~ job + education + campaign + poutcome + 
                      ...
                      education*nr.employed, family="binomial", data=logit.os.Train)
...
obj1.mod2.acc <- 0.7945
obj1.mod2.sens <- 0.8223
obj1.mod2.spec <- 0.5754
...
## Simple with original
## Accuracy - 88.9%, Sensitivity - 98.7, Specificity - 18.4
```{r}
set.seed(123)
...
# Confusion Matrix
simple.o.cm <- confusionMatrix(as.factor(simple.o.pred), as.factor(simple.o.test$y))
simple.o.cm
...
simple.o.sens <- 0.9867
simple.o.sens 
simple.o.spec <- 0.1842
simple.o.spec
...
## Accuracy - 67.4%, Sensitivity - 67.9, Specificity - 63.0
```{r}
set.seed(123)
...
# Confusion Matrix
simple.b.cm <- confusionMatrix(as.factor(simple.b.pred), as.factor(simple.b.test$y))
simple.b.cm
...
# Sensitivity and Specificity
simple.b.sens <- 0.9867
simple.b.sens 
simple.b.spec <- 0.1842
simple.b.spec
...
vif(simple.b.lm)
```
## Simple with rose 83.28
## Accuracy - 66.8%, Sensitivity - 67.4, Specificity - 62.3
```{r}
set.seed(123)
...
# Confusion Matrix
simple.r.cm <- confusionMatrix(as.factor(simple.r.pred), as.factor(simple.r.test$y))
simple.r.cm
...
# Sensitivity and Specificity
simple.r.sens <- 0.6738
simple.r.sens 
simple.r.spec <- 0.6228
simple.r.spec
...
## Accuracy - 88.9%, Sensitivity - 95.9%, Specificity - 32.9%
```{r}
set.seed(123)
...
## Accuracy - 72.4%, Sensitivity - 72.7%, Specificity - 69.9%
```{r}
set.seed(123)
...
## Accuracy - 72.5%, Sensitivity - 72.6%, Specificity - 71.8%
```{r}
set.seed(123)
...
## Accuracy - 87.5%, Sensitivity - 92.2, Specificity - 48.9
```{r}
set.seed(123)
...
## Accuracy - 87.1%, Sensitivity - 91.3, Specificity - 53.3
```{r}
set.seed(123)
...
## Accuracy - 87.6%, Sensitivity - 92.7, Specificity - 46.2
```{r}
set.seed(123)
...
```
## MDA with balanced
## Accuracy - 68.9%, Sensitivity - 69.7%, Specificity - 63.0%
```{r}
set.seed(123)
...
```
## MDA with rose
## Accuracy - 77.5%, Sensitivity - 80.8%, Specificity - 50.7%
```{r}
set.seed(123)
...
## FDA with original
## Accuracy - 89.8%, Sensitivity - 98.3%, Specificity - 21.1%
```{r}
set.seed(123)
...
```
## FDA with balanced
## Accuracy - 68.3%, Sensitivity - 69.2%, Specificity - 61.7%
```{r}
set.seed(123)
...
```
## FDA with rose
## Accuracy - 67.5%, Sensitivity - 68.2%, Specificity - 61.8%
```{r}
set.seed(123)
...
## RDA with original
## Accuracy - 89.8%, Sensitivity - 98.3%, Specificity - 20.8%
```{r}
set.seed(123)
...
```
## RDA with rose
## Accuracy - 87.9%, Sensitivity - 94.3%, Specificity - 35.4%
```{r}
set.seed(123)
...
## RF with original
## Accuracy - 89.7%, Sensitivity - 98.6%, Specificity - 19.5%
```{r}
# Split the data into training (70%) and test set (30%)
set.seed(123)
rf.o.data <- obj2.data
...
```
## RF with balanced
## Accuracy - 85.2%, Sensitivity - 87.6%, Specificity - 66.2%
```{r}
# Split the data into training (70%) and test set (30%)
set.seed(123)
rf.b.data <- obj2.data
...
```
## RF with rose
## Accuracy - 89.4%, Sensitivity - 97.4%, Specificity - 26.25%
```{r}
# Split the data into training (70%) and test set (30%)
set.seed(123)
rf.r.data <- obj2.data
...
## Ridge with original
## Accuracy - 89.9%, Sensitivity - 98.7%, Specificity - 18.3%
```{r}
set.seed(123)
...
OBJ12.results <- cbind("Complex Obj 1 Model", obj1.mod2.acc, obj1.mod2.sens, obj1.mod2.spec)
...
accuracy <- rbind(OBJ1.results, OBJ12.results,
