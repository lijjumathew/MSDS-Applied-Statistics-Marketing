...
```{r}
step.os6.res <- cbind("Model 6",obj1.mod6.acc, obj1.mod6.sens, obj1.mod6.spec, vif.6.max, obj1.mod6.msclc, auc.6)
step.os1.res <- cbind("Model 1",obj1.mod1.acc, obj1.mod1.sens, obj1.mod1.spec, vif.1.max, obj1.mod1.msclc, auc1)
step.os2.res <- cbind("Model 2",obj1.mod2.acc, obj1.mod2.sens, obj1.mod2.spec, vif.2.max, obj1.mod2.msclc, auc2)
step.os4.res <- cbind("Model 4",obj1.mod4.acc, obj1.mod4.sens, obj1.mod4.spec, vif.4.max, obj1.mod4.msclc, auc4)
step.os7.res <- cbind("Model 7",obj1.mod7.acc, obj1.mod7.sens, obj1.mod7.spec, vif.7.max, obj1.mod7.msclc, auc7)
step.os3.res <- cbind("Model 3",obj1.mod3.acc, obj1.mod3.sens, obj1.mod3.spec, vif.3.max, obj1.mod3.msclc, auc3)
step.os5.res <- cbind("Model 5",obj1.mod5.acc, obj1.mod5.sens, obj1.mod5.spec, vif.5.max, obj1.mod5.msclc, auc5)


accuracy1 <- rbind(step.os6.res, step.os1.res, step.os2.res, step.os4.res, step.os7.res, step.os3.res, step.os5.res)

colnames(accuracy1) <- c("Method", "Accuracy", "Sensitivity", "Specificity", "LargestVIF" , "MiscalcRate", "AUCROC")

accuracytbl1 <- as.data.frame(accuracy1)
accuracytbl1$Accuracy <- as.numeric(accuracytbl1$Accuracy)
accuracytbl1$Sensitivity <- as.numeric(accuracytbl1$Sensitivity)
accuracytbl1$Specificity <- as.numeric(accuracytbl1$Specificity)
accuracytbl1$LargestVIF <- as.numeric(accuracytbl1$LargestVIF)
accuracytbl1$MiscalcRate <- as.numeric(accuracytbl1$MiscalcRate)
accuracytbl1$AUCROC <- as.numeric(accuracytbl1$AUCROC)
...
options(digits = 3)
as.datatable(formattable(accuracytbl1,align = c("l",rep("r", NCOL(accuracytbl1) - 1)),
            list(`Method` = formatter("span", style = ~ style(color = "black", font.weight = "bold")), 
                 ...
                 'MiscalcRate' = color_text(customGreen, customRed),
                 'LargestVIF' = color_text(customGreen, customRed),
                 "AUCROC" = color_text(customRed, customGreen)))
)
```
...
lY_data <- filter(lda.qda.data,grepl("2",y))

#Seperate all "no's" in dataset
lN_data <- filter(lda.qda.data,grepl("1", y))
...
BalanceIndex <- createDataPartition(obj2.data$y, p = .7,
                                  ...
                                  times = 1)
#Seperate all 4640 "yes" in dataset
Y_data <- filter(obj2.data,grepl("2",y))

#Seperate all "no's" in dataset
N_data <- filter(obj2.data,grepl("1",y))
...
rf.o.train.data <- orig.train
rf.o.train.data$y <- factor(rf.o.train.data$y)
rf.o.test.data <- orig.test
...
rf.b.train.data <- balanced.train
rf.b.train.data$y <- factor(rf.b.train.data$y)
rf.b.test.data <- balanced.test
...
rf.r.train.data <- rose.train
rf.r.train.data$y <- factor(rf.r.train.data$y)
rf.r.test.data <- rose.test
...
# Error
lasso.o.error <- mean(lasso.o.test.data$y != class.o.lasso)
lasso.o.error
...
# Error
lasso.b.error <- mean(lasso.b.test.data$y != class.b.lasso)
lasso.b.error
...
# Error
lasso.r.error <- mean(lasso.r.test.data$y != class.r.lasso)
lasso.r.error
