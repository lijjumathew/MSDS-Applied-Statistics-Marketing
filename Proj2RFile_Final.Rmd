...
print("Confusion matrix for Stepwise")
obj1.mod.cm <- confusionMatrix(as.factor(class.step), logit.os.Test$y)

obj1.mod.acc <- obj1.mod.cm$overall["Accuracy"]
obj1.mod.acc
obj1.mod.sens <- obj1.mod.cm$byClass["Sensitivity"]
obj1.mod.sens
obj1.mod.spec <- obj1.mod.cm$byClass["Specificity"]
obj1.mod.spec
...
class.step <- factor(ifelse(fit.pred.step > cutoff,"yes","no"))
print("Confusion matrix for Stepwise")
obj1.mod2.cm <- confusionMatrix(as.factor(class.step), logit.os.Test$y)
obj1.mod2.cm
...
cutoff<-0.5
class.o.lasso<-factor(ifelse(fit.o.pred.lasso>cutoff,"yes","no"))
...
as.datatable(formattable(accuracytbl,align = c("l",rep("r", NCOL(accuracytbl) - 1)),
            list(`Method` = formatter("span", style = ~ style(color = "black", font.weight = "bold")), 
                 ...
                 'Error Rate' = color_text(customRed, customGreen)))
)
```
...
RDA.O.results <- cbind("RDA Original",rda.o.acc, rda.o.sens, rda.o.spec, rda.o.error)
RDA.B.results <- cbind("RDA Balanced",rda.b.acc, rda.b.sens, rda.b.spec, rda.b.error)
RDA.R.results <- cbind("RDA Rose",rda.r.acc, rda.r.sens, rda.r.spec, rda.r.error)
...
as.datatable(formattable(accuracytbl, digits = 3,
            ...
            list(`Method` = formatter("span", style = ~ style(color = "black", font.weight = "bold")), 
                 ...
                 'Sensitivity' = color_text(customRed, customGreen),
                 "Specificity" = color_text(customRed, customGreen),
                 "ErrorRate" = color_text(customRed, customGreen)))
)
```
